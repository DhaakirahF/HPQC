...
int main(int argc, char **argv)
{
    ...
    int *my_vector = NULL;
    int start, end;
    long long local_sum = 0, global_sum = 0, recv_sum = 0;
    double t1, t2;
    MPI_Status status;
    ...
    if (my_rank == 0)
    {
        ...
        for (int src = 1; src < uni_size; src++)
        {
            MPI_Recv(&recv_sum, 1, MPI_LONG_LONG, src, 0, MPI_COMM_WORLD, &status);
            global_sum += recv_sum;
        }
    }
    else
    ...
    if (my_rank == 0)
    {
        printf("Manual send/recv version\n");
        printf("Array size = %d\n", num_arg);
        printf("Processes  = %d\n", uni_size);
        printf("Sum        = %lld\n", global_sum);
...
void fill_vector(int vector[], int size)
{
    for (int i = 0; i < size; i++) vector[i] = i * i + 1;
}
